...
require_relative "sanitize_url/version"
...
module SanitizeUrl
  ...
  def sanitize_url(url, options = {})
    raise(ArgumentError, "options[:schemes] must be an array") if options.has_key?(:schemes) && !options[:schemes].is_a?(Array)
    options = {
      replace_evil_with: "",
      default_scheme: "http",
    ...
    matches = url.match(/^(.+?)(:|%3A)(.*)$/)
    dirty_scheme = matches[1] if matches
    if dirty_scheme
      unescaped_opaque = matches[3]
      return options[:replace_evil_with] if unescaped_opaque.nil? || unescaped_opaque.empty? || unescaped_opaque.match(/^\/+$/)
    else
    ...
    dirty_scheme = dirty_scheme.gsub(/%([a-zA-Z0-9]{2})/) do
      code = $1.to_i(16)
    ...
    unescaped_opaque.each_byte do |code|
      opaque += if SanitizeUrl.url_encode?(code)
        "%" + code.to_s(16).upcase
      else
        code.chr
      end
    ...
    if options[:schemes].collect { |s| s.to_s }.include?(scheme.downcase)
      if HTTP_STYLE_SCHEMES.include?(scheme.downcase) && !opaque.match(/^\/\//)
  ...
  def self.dereference_numerics(str) # :nodoc:
    ...
    str = str.gsub(/&#([a-fA-f0-9]+);?/) do  # standard:disable Lint/MixedCaseRange
      char_or_url_encoded($1.to_i)
    end
    # Hex code points, e.g. &#x6A; &#x6A
    str.gsub(/&#[xX]([a-fA-f0-9]+);?/) do  # standard:disable Lint/MixedCaseRange
  ...
  def self.url_encode?(code) # :nodoc:
    !(
      code.between?(48, 57) or   # Numbers
      code.between?(65, 90) or   # Uppercase
      code.between?(97, 122) or   # Lowercase
