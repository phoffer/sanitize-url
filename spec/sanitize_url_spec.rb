...
RSpec.describe SanitizeUrl do
  include SanitizeUrl

  it "has a version number" do
    expect(SanitizeUrl::VERSION).not_to be nil
  end
  ...
  describe "#sanitize_url" do
    it "replaces JavaScript URLs with options[:replace_evil_with]" do
      [
        ...
        expect(sanitize_url(evil_url, replace_evil_with: "replaced")).to eq("replaced")
      end
    end
    ...
    context "with :schemes whitelist" do
      ...
      it "allows anything on the list" do
        [
          ...
          expect(sanitize_url(good_url, schemes: ["http", "https"])).to eq(good_url)
        end
      end

      it "works with schemes given as symbols" do
        expect(sanitize_url("ftp://example.com", schemes: [:http, :https], replace_evil_with: "replaced")).to eq("replaced")
        expect(sanitize_url("ftp://example.com", schemes: [:http, :https, :ftp])).to eq("ftp://example.com")
      end
    end

    it "prepends http:// if no scheme is given" do
      expect(sanitize_url("www.example.com")).to eq("http://www.example.com")
    end

    it "prepends default_scheme option if no scheme is given and default_scheme option is set" do
      expect(sanitize_url("www.example.com", default_scheme: "https")).to eq("https://www.example.com")
    end
    ...
    it "replaces evil URLs that are URL-encoded (hex with %)" do
      expect(sanitize_url("%6A%61%76%61%73%63%72%69%70%74%3A%61%6C%65%72%74%28%22%58%53%53%22%29", replace_evil_with: "replaced")).to eq("replaced")
    end

    it "does not try to fix broken schemes after the start of the string" do
      expect(sanitize_url("http://example.com/http/foo")).to eq("http://example.com/http/foo")
    end

    it "does not prepend an extra http:// if a valid scheme is given" do
      expect(sanitize_url("http://www.example.com")).to eq("http://www.example.com")
      expect(sanitize_url("https://www.example.com")).to eq("https://www.example.com")
      expect(sanitize_url("ftp://www.example.com")).to eq("ftp://www.example.com")
    end

    it "dereferences URL-encoded characters in the scheme" do
      expect(sanitize_url("h%74tp://example.com")).to eq("http://example.com")
    end

    it "dereferences decimal numeric character references in the scheme" do
      expect(sanitize_url("h&#116;tp://example.com")).to eq("http://example.com")
    end
    ...
    it "retains URL-encoded characters in the opaque portion" do
      expect(sanitize_url("http://someone%40gmail.com:password@example.com")).to eq("http://someone%40gmail.com:password@example.com")
    end
    ...
    it "URL-encodes code points outside ASCII" do
      ...
      expect(sanitize_url("http://&#1044;")).to eq("http://%D0%94")
      expect(sanitize_url("http://&#x0414;")).to eq("http://%D0%94")
      expect(sanitize_url("http://\xD0\x94")).to eq("http://%D0%94") # UTF-8 version of the same.
    end

    it "replaces URLs without the opaque portion" do
      expect(sanitize_url("http://", replace_evil_with: "replaced")).to eq("replaced")
      expect(sanitize_url("mailto:", replace_evil_with: "replaced")).to eq("replaced")
    end

    it "adds the two slashes for known schemes that require it" do
      expect(sanitize_url("http:example.com")).to eq("http://example.com")
      expect(sanitize_url("ftp:example.com")).to eq("ftp://example.com")
      expect(sanitize_url("svn+ssh:example.com")).to eq("svn+ssh://example.com")
    end

    it "does not add slashes for schemes that do not require it" do
      expect(sanitize_url("mailto:someone@example.com")).to eq("mailto:someone@example.com")
    end

    it "strips invalid characters from the scheme and then evaluates the scheme according to the normal rules" do
      expect(sanitize_url("ht\xD0\x94tp://example.com")).to eq("http://example.com")
      expect(sanitize_url("htt$p://example.com")).to eq("http://example.com")
      expect(sanitize_url('j%avascript:alert("XSS")', replace_evil_with: "replaced")).to eq("replaced")
    end
  end

  describe ".dereference_numerics" do
    it "decodes short-form decimal UTF-8 character references with a semicolon" do
      expect(SanitizeUrl.dereference_numerics("&#106;")).to eq("j")
    end

    it "decodes short-form decimal UTF-8 character references without a semicolon" do
      expect(SanitizeUrl.dereference_numerics("&#106")).to eq("j")
    end
    ...
    it "decodes long-form decimal UTF-8 character references without a semicolon" do
      expect(SanitizeUrl.dereference_numerics("&#0000106")).to eq("j")
    end
    ...
    it "decodes hex UTF-8 character references without a semicolon" do
      expect(SanitizeUrl.dereference_numerics("&#x6A")).to eq("j")
    end
  end
end
